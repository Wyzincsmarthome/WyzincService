...
function processProductPrices(product) {
    ...
    let retailPrice = parseFloat(String(product.price || product.pvpr || '0').replace(/[^0-9.,]/g, '').replace(',', '.')) || 0;
...
// Função principal reescrita com a lógica final e correta
async function createProductToShopify(shopifyClient, product, tags) {
    ...
    try {
        ...
        const createInput = { input: { title: product.name, status: 'ACTIVE' } };
        ...
        if (!createdProduct || !createdProduct.id || !createdProduct.variants.edges[0]?.node?.id) {
            throw new Error('Falha ao obter IDs do produto ou da variante criados no Passo 1.');
        }
        ...
        // CORREÇÃO FINAL: Usar a mutação 'productUpdate' para adicionar todos os detalhes
        const productUpdateMutation = `
            mutation productUpdate($input: ProductInput!) {
                productUpdate(input: $input) {
                    product { id, title, variants(first: 1) { edges { node { id, sku, price } } } }
                    userErrors { field, message }
                }
            }`;
        ...
        const updateInput = {
            input: {
                ...
                variants: [{
                    id: variantId, // Especificar o ID da variante a atualizar
        ...
        const updatedProduct = updateResponse.data.data.productUpdate.product;
        if (!updatedProduct) throw new Error('Falha ao atualizar o produto no Passo 2.');

        console.log(`🎉 PRODUTO COMPLETO "${updatedProduct.title}" CRIADO E ATUALIZADO COM SUCESSO!`.green.bold);
